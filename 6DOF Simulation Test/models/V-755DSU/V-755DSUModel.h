...
class V755DSU{
    ...
    void motor(double TOF, double dt){  //Thrust level then fuel mass change
        if(TOF <= 3.0){thrust = 460000.0 - 130000.0*(TOF/3); motorData[0] -= 174*dt;}  //Boost main phase
            ...
            else if(TOF > 45.0 && TOF <= 45.5){thrust = -17500.0*(TOF-45.0) + 35000.0; motorData[2] -= 723.4/42*dt;}   //Sustainer transition
    ...
    void initialState(){
        ...
        modelData[12] = tau_rudder;
        modelData[13] = thrust;
        modelData[14] = C_DragBase;
        modelData[15] = C_LiftBase;
        modelData[16] = CG_CP_avg;
        modelData[17] = tau_aileron;
        modelData[18] = deltaAmax;
        modelData[19] = deltaERmax;
    }
    void newState(){
        modelData[0] = mass;
        for(int i = 0; i < 4; ++i){modelData[i+1] = inertiaData[i];}
    ...
    const double deg_rad{M_PI/180.0}, slug_kg{14.5939}, inch_m{0.0254}, sqft_sqm{0.092903}, ft_m{0.304878}, 
    slugsqft_kgsqm{1.3558179619}, tau_rudder{0.1}, tau_aileron{0.05}, C_LiftBase{0.0}, C_DragBase{0.27},
    deltaAmax{12*deg_rad}, deltaERmax{28*deg_rad},
    ...
    emptyMass = 667.0;
    double mass{0}, length{10.778}, radius{0.327},  thrust{0},  CG_CP_avg{0.8065},
        modelData[20],
