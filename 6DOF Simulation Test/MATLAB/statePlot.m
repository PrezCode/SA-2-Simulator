...
%Angular Rates Plot
figure(2);
plot(time, P*180/pi, '-','Color',"r"); 
hold on; 
plot(time, Q*180/pi, '--','Color',"g");
plot(time, R*180/pi,'-.','Color',"b");
hold off;
grid on;
xlabel('Seconds');
...
% set(gca,'XLim', [0 15000],'YLim',[0 15000], 'ZLim',[0 2500]);
% view(-45,45);
% hold on;
% grid on;
% xlabel('North (m)');
...
plot(time, Clsf, '--','Color',"g");
hold off;

grid on;
xlabel('Seconds');
title('Aerodynamic Coefficients over Time');
legend('Drag', 'Lift_a', 'Lift_b', 'Sideforce');
