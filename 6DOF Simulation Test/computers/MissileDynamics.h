...
class MissileDynamics{
    ...
    enum DataType{Mass = 0, Ix = 1, Iy = 2, Iz = 3, Ixz = 4, Length = 5, Radius = 6, Span = 7, Chord = 8, AileronARef = 9, 
        WingARef = 10, RudderARef = 11, RudderTau = 12, Thrust = 13, CDrag = 14, CLift = 15, CGCP = 16};
    ...
    void externalForces(){
        ...
        OZ_meas = Fb[y]/mass/g;   //Felt lateral acceleration in the OZ axis, traditionally the East/Y axis of the body
        OY_meas = Fb[z]/mass/g;   //Felt lateral acceleration in the OY axis, traditionall the Down/Z axis of the body
        moments[L] = MomentCoefficient(L)*dynamicPressure*(diameter)*Aref;
        moments[M] = MomentCoefficient(M)*dynamicPressure*(diameter)*Aref;
        moments[N] = MomentCoefficient(N)*dynamicPressure*(diameter)*Aref;
    }
    ...
    void derivatives(){
        ...
        instanceDeriv[12] = (deltaCMD[0] - instance[12])/tau_rudder;    //aileron
        instanceDeriv[13] = (deltaCMD[1] - instance[13])/tau_rudder;    //elevator
        instanceDeriv[14] = (deltaCMD[2] - instance[14])/tau_rudder;    //rudder
    }
    ...
    double getQuickData(){return Fb[z];}
    private:
    bool atmosphereON;
    ...
    double Cm_bn[3][3], Cm_bnT[3][3],Cm_wb[3][3], Cm_wbT[3][3],   //Cosine matrices for body-normal, normal-body, wind-body
            ...
            Aref, Aref_aileron, Aref_wing, Aref_rudder, C_DragBase, C_LiftBase, C_Drag, C_LiftPitch, C_LiftYaw, diameter{0}, 
            CnBeta{0}, Cmq{0}, Clp{0}, Cnr{0}, CmAoA{0}, volume, CndelR{0}, CmdelE{0}, CldelA{0}, d2r{M_PI/180.0}, C_Nalpha{0}, K_D{0}, OZ_meas{0.0}, OY_meas{0.0},
    ...
    double MomentCoefficient(Variables axis){
        ...
        if(Vmag > 0){
            switch(axis){
                ...
                case M: return CmAoA*AoA + Cmq*(W[q]*diameter)/(2.0*Vmag) + CmdelE*instance[13]; break;
                case N: return CnBeta*sideslip + Cnr*(W[r]*diameter)/(2.0*Vmag) + CndelR*instance[14]; break;
            }
        }
        return 0;
    }
